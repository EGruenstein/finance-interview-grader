...
if "questions" in st.session_state:
    ...
    # ✅ CHECK IF INTERVIEW IS DONE FIRST
    if st.session_state.current_index >= len(questions):
        ...
        if st.button("Restart Interview"):
            ...
            st.rerun()
        st.stop()  # 🔒 prevents the rest from running

    # Otherwise, show current question
    index = st.session_state.current_index
    q = questions[index]

    st.title(f"💼 Finance Interview — Question {index + 1} of {len(questions)}")
    st.markdown(f"### {q['question']}")
    st.session_state.answers[index] = st.text_area("Your Answer:", value=st.session_state.answers[index], key=f"answer_{index}")
    ...
    if st.button("Submit Answer"):
        with st.spinner("Grading..."):
            st.session_state.feedback[index] = grade_answer(
                q["question"], q["example"], st.session_state.answers[index], client
            )
            st.session_state.current_index += 1
            st.rerun()

    if st.session_state.feedback[index]:
